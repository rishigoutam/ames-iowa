...
#2 Construct Age and delete YearBuilt

df_cleaned$Age = as.integer(year(df_cleaned$DateSold) - df_cleaned$YearBuilt)
df_cleaned = select(df_cleaned, -YearBuilt)

#3 Construct YrsSinceRemod and delete YearRemodAdd

df_cleaned$YrsSinceRemod = as.integer(year(df_cleaned$DateSold) - df_cleaned$YearRemodAdd)
...
df_cleaned$LotConfig_temp[df_cleaned$LotConfig == "Inside"] = as.integer(0)
df_cleaned$LotConfig_temp[df_cleaned$LotConfig == "Corner"] = as.integer(1)
df_cleaned$LotConfig_temp[df_cleaned$LotConfig == "CulDSac"] = as.integer(2)
df_cleaned$LotConfig_temp[df_cleaned$LotConfig == "FR2"] = as.integer(3)
df_cleaned$LotConfig_temp[df_cleaned$LotConfig == "FR3"] = as.integer(4)
...
df_cleaned = df_cleaned %>%
  ...
  relocate(YrsSinceRemod, .after = Neighborhood) %>%   
...
df_temp = df_cleaned %>%
  select(PID, DateSold, Rate, logSalePrice, IsOnMainroad, IsNearPark, IsNearRailroad, LotConfig, LandSlope, LandContour, LotShape, StreetAlley, YrsSinceRemod, Age)
...
fig5 = ggscatter(df_cleaned, x = "Age", y = "logSalePrice", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          size = 0.25, color = "blue",
...
cat("mean(YrsSinceRemod) = ", mean(df_cleaned$YrsSinceRemod), "\n") 
cat("sd(YrsSinceRemod) = ", sd(df_cleaned$YrsSinceRemod), "\n")

fig6 = ggscatter(df_cleaned, x = "YrsSinceRemod", y = "logSalePrice", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          size = 0.25, color = "blue",
          cor.coef.coord = c(0,4.25),
          xtickslab.rt = 45,
          xlab = "Period Since Remodel (years)", ylab = "SalePrice (USD)")
