...
df_Elastic_Engineered = select(df_akram, -c(PID, GarageType, Exterior2nd, Neighborhood_Int))
...
Calculation of AIC and BIC for Elastic Net Model:

```{r}

# Calculation of AIC and BIC for Elastic_Model - the hard way

#tLL = Elastic_Model$finalModel$nulldev - min(deviance(Elastic_Model$finalModel))

tLL = Elastic_Model$finalModel$nulldev - deviance(Elastic_Model$finalModel)

#k = Elastic_Model$finalModel$df[which.min(deviance(Elastic_Model$finalModel))]

k = Elastic_Model$finalModel$df

n = Elastic_Model$finalModel$nobs

AICc_Elastic = -tLL + 2*k + 2*k*(k+1)/(n-k-1)

BIC_Elastic = log(n)*k - tLL

AICc_Elastic

BIC_Elastic

predict(Elastic_Model$finalModel, type = "coefficients")

Elastic_Model$finalModel$nulldev

min(deviance(Elastic_Model$finalModel))
